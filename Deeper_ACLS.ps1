...
function deeper-acls($filter){
            Get-DomainObjectAcl | select -expandProperty ObjectDN  | Get-Unique | % {$permissions=@(Get-Acl AD:\$_ | select -ExpandProperty Access | ?{$_.IdentityReference -like "*$filter*"} | Format-Table);if($permissions){Write-Host $_; Get-ADObject -SearchBase $_ | select name, objectclass ;$permissions}}
}
...
foreach($user in $users.Split(" ")){
    ...
    Get-ADPrincipalGroupMembershipRecursive "$user" | %{ $filter=$_.Split("=")[1].Split(",")[0]; Write-Host "ACLS for membership $user - $_ - $filter"; deeper-acls $filter} 
